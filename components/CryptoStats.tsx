...
export default function CryptoStats() {
  ...
  return (
    <div className="grid grid-cols-1 gap-12 md:grid-cols-2 xl:grid-cols-3 xl:gap-16">
      {!isLoading ? (
        <>
          <CryptoTable heading="Trending" data={data?.trending} />
          <CryptoTable heading="Top Gainers" data={data?.topGainers} />
          <CryptoTable heading="Recently Added" data={data?.recentlyAdded} />
        </>
      ) : (
        <>
